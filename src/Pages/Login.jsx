...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://eventmanagementserver-3qnz.onrender.com/auth/login", formData);
      ...
      showToast("error", err.response?.data?.message || "Authentication failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="min-h-screen flex items-center justify-center bg-black px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        {/* Login Card */}
        <div className="bg-gray-900/50 border border-gray-800 backdrop-blur-xl rounded-[2.5rem] p-10 shadow-2xl">
          <div className="text-center mb-10">
            <h2 className="text-4xl font-black text-white tracking-tighter mb-2">
              EVENTHUB<span className="text-yellow-400">.</span>
            </h2>
            <p className="text-gray-400 text-sm font-medium uppercase tracking-[0.2em]">Secure Login</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-[10px] font-bold text-yellow-400/60 hover:text-yellow-400 cursor-pointer transition">Forgot?</span>
              </div>
              <input
                type="password"
                name="password"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-black py-4 rounded-2xl
            ...
            <div className="pt-6 text-center">
              <p className="text-sm text-gray-500">
                ...
                <button
                  ...
                  className="text-yellow-400 font-bold hover:underline underline-offset-4"
                >
                  Create Account
                </button>
              </p>
